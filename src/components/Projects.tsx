...
export const Products = () => {
  ...
  return (
    <section id="products" className="py-32 px-4 sm:px-6 lg:px-8 bg-background relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((product, index) => (
            <Card 
              key={index} 
              className={`group bg-card/50 backdrop-blur-sm hover:bg-card/70 transition-all duration-500 hover:-translate-y-4 border ${product.borderColor} overflow-hidden cursor-pointer`}
              onClick={() => window.location.href = '/products'}
            >
              <CardContent className="p-8">
                <div className={`w-20 h-20 bg-gradient-to-r ${product.color} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500`}>
